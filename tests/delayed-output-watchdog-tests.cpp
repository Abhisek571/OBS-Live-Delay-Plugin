...
{
	try {
		require(evaluate_delayed_output_health(true, 0, 1s, 5s) == DelayedOutputHealth::WaitingForVideo,
			"an active output should receive a startup grace period");
		require(evaluate_delayed_output_health(true, 1, 1s, 5s) == DelayedOutputHealth::Progressing,
			"the first encoded video frame should satisfy the watchdog");
		require(evaluate_delayed_output_health(false, 0, 1s, 5s) == DelayedOutputHealth::Stopped,
			"an asynchronously stopped output should trigger recovery");
		require(evaluate_delayed_output_health(true, 0, 5s, 5s) == DelayedOutputHealth::VideoTimeout,
